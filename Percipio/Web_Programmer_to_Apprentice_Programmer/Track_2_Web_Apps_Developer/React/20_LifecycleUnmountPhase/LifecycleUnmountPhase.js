        ...
        return (
            <div>
                {this.state.names.map((name) =>
                    <Employee key= {index++} name={name} deleteRandom={this.deleteRandom}/>
                )}

                <button className="button" onClick={this.deleteRandom}>Delete employee</button>
            </div>
        );
    }
}
...
ReactDOM.render(<EmployeeList initialNames={["Mark", "Greg", "Tim", "Olaf", "Peter"]}/>, document.getElementById('root'));